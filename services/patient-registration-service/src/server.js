	...
	async (req, res, next) => {
		try {
			...
			const unknownFields = providedFields.filter(
				(field) => !allowedPatientRegistrationFields.has(field),
			);
			...
			if (!/^\d{15}$/.test(normalizedEmiratesId)) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"emiratesId must contain exactly 15 digits",
				);
			}
			...
			if (!normalizedEntryRoute) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"entryRoute must be either OPD or A&E",
				);
			}
			...
			if (existingPatient) {
				return sendError(
					...
					409,
					"DUPLICATE_PATIENT",
					"A patient with this Emirates ID already exists",
				);
			}
			...
			if (!patientRecord.source) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"source must be one of manual, device, api",
				);
			}
...
app.patch(
	...
	async (req, res, next) => {
		try {
			...
			const providedFields = Object.keys(req.body || {});
			const unknownFields = providedFields.filter(
				(field) => !allowedVisitFields.has(field),
			);
			...
			if (!isStringArray(diseases)) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"diseases must be an array of non-empty strings",
				);
			}
			...
			if (entryRoute !== undefined && !visitEntryRoute) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"entryRoute must be a non-empty string when provided",
				);
			}
	...
	async (req, res, next) => {
		try {
			...
			const scheduleUpdate = {
				doctorId,
				doctorName:
					typeof doctorName === "string" ? doctorName.trim() : undefined,
				department:
...
app.get(
	...
	async (req, res, next) => {
		try {
			...
			const dayEntry = schedule.weeklyAvailability.find(
				(entry) => entry.dayOfWeek === dayOfWeek,
			);
			const availableSlots = dayEntry?.slots?.length
				? dayEntry.slots
				: buildDefaultHalfHourSlots();
		...
		try {
			...
			if (!appointmentDateTime || typeof appointmentDateTime !== "string") {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"appointmentDateTime is required as an ISO date-time string",
				);
			}
			...
			if (
				...
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"durationMinutes must be an integer between 5 and 480",
				);
			}
			...
			if (Number.isNaN(appointmentDate.getTime())) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"appointmentDateTime must be a valid ISO date-time string",
				);
			}
			...
			if (appointmentDate.getUTCDate() !== appointmentEndDate.getUTCDate()) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"Appointment cannot cross midnight; split into separate appointments",
				);
			}
			...
			if (!doctorSchedule) {
				return sendError(
					...
					404,
					"DOCTOR_SCHEDULE_NOT_FOUND",
					"Doctor schedule not found for this doctor",
				);
			}

			const dayOfWeek = appointmentDate.getUTCDay();
			const daySchedule = doctorSchedule.weeklyAvailability.find(
				(entry) => entry.dayOfWeek === dayOfWeek,
			);
			const scheduleSlots = daySchedule?.slots?.length
				? daySchedule.slots
				: buildDefaultHalfHourSlots();
...
app.patch(
	...
	async (req, res, next) => {
		try {
			...
			const providedFields = Object.keys(req.body || {});
			const unknownFields = providedFields.filter(
				(field) => !allowedPrescriptionFields.has(field),
			);
			...
			if (!isStringArray(medicines)) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"medicines must be an array of non-empty strings",
				);
			}
			...
			if (typeof prescribedAt !== "string" || !prescribedAt.trim()) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"prescribedAt is required and must be an ISO date-time string",
				);
			}
			...
			if (Number.isNaN(parsedPrescribedAt.getTime())) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"prescribedAt must be a valid ISO date-time string",
				);
			}
			...
			if (notes !== undefined && (typeof notes !== "string" || !notes.trim())) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"notes, when provided, must be a non-empty string",
				);
			}
...
app.patch(
	...
	async (req, res, next) => {
		try {
			...
			const providedFields = Object.keys(req.body || {});
			const unknownFields = providedFields.filter(
				(field) => !allowedNursingFields.has(field),
			);
			...
			const { treatmentDetails, intakeOutput, recordedAt, source } =
				req.body || {};
			...
			if (typeof recordedAt !== "string" || !recordedAt.trim()) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"recordedAt is required and must be an ISO date-time string",
				);
			}
			...
			if (Number.isNaN(parsedRecordedAt.getTime())) {
				return sendError(
					...
					400,
					"VALIDATION_ERROR",
					"recordedAt must be a valid ISO date-time string",
				);
			}
...
connectDatabase()
	...
	.catch((error) => {
		console.error(
			"[Patient Registration] Failed to connect to MongoDB:",
			error.message,
		);
		process.exit(1);
	});
