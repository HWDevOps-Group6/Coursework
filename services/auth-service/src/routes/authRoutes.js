...
router.get(
	"/staff/doctors",
	authenticate,
	authorizeRoles("clerk", "admin", "doctor", "nurse", "paramedic", "clinician"),
	async (req, res) => {
		try {
...
router.post("/verify", (req, res) => {
	const token =
		extractTokenFromHeader(req.headers.authorization) || req.body?.token;
	if (!token) {
		return sendError(
			res,
			400,
			"TOKEN_REQUIRED",
			"Token required in Authorization header or body",
		);
	}
	try {
