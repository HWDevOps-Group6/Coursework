...
router.get(
	...
	passport.authenticate("google", {
		session: false,
		failureRedirect: "/api/auth/web/google/failure",
	}),
	(req, res) => {
		const { token } = req.user;
		const redirectUrl =
			process.env.GOOGLE_REDIRECT_AFTER_LOGIN || "http://localhost:5173";
...
router.get("/google/failure", (req, res) => {
	return sendError(
		...
		401,
		"GOOGLE_AUTH_FAILED",
		"Google sign-in failed. This flow requires a browser.",
	);
});
